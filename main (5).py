...
async def commands(ctx):
    ...
    embed.add_field(
        ...
        value="`!create #channel` - Create tournament\n`!start` - Start tournament\n`!cancel` - Cancel tournament\n`!winner @player` - Set match winner\n`!code <code>` - Send room code",
        inline=False
    )
    
    embed.add_field(
        name="🎯 Host Commands", 
        value="`!hosterregist <max>` - Start host registration\n`!fake <number>` - Add fake players",
        inline=False
    )
    
    embed.add_field(
        name="🏷️ Bracket Commands",
    ...
    embed.add_field(
        ...
        value="`!sp @user` - Check SP\n`!sp_lb` - SP leaderboard\n`!sp_rst` - Reset all SP (admin)",
        inline=False
    )
    
    embed.add_field(
        name="⚙️ Permission Commands",
        value="`!htr @role` - HTR permissions\n`!adr @role` - ADR permissions\n`!tlr @role` - TLR permissions",
        inline=False
    )
