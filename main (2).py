...
class Tournament:
    def __init__(self):
        ...
        self.prize = ""
        self.title = ""
        self.finalists = [] # Stores the finalists before the bronze medal battle
        self.bronze_winner = None # Stores the winner of the bronze medal battle
...
# Load data
def load_data():
    global user_data, user_levels, leveling_settings, welcomer_settings, tp_data, bracket_roles, automod_settings, sp_data, host_roles, admin_roles, tournament_leader_roles
    try:
        with open('user_data.json', 'r') as f:
            ...
            tournament_leader_roles = data.get('tournament_leader_roles', {})
...
def save_data():
    data = {
        ...
        'tournament_leader_roles': tournament_leader_roles
    }
...
class TournamentView(discord.ui.View):
    ...
    async def unregister_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            await interaction.response.edit_message(view=self)
            await interaction.followup.send(f"✅ {interaction.user.display_name} unregistered! ({item.label})", ephemeral=True)
...
async def svd(ctx):
    ...
    # Check if user has permission (bot owner or special user)
    if ctx.author.id != 1011738066962989056 and not await bot.is_owner(ctx.author):
...
tournament_leader_roles = {}  # {guild_id: [role_ids]}

def has_host_permission(member, guild):
    """Check if member has host permission (manage_channels, manage_messages, assigned host role, or special user)"""
    # Special user with all permissions
    if member.id == 1011738066962989056:  # @n_x_pedroz1's user ID
        return True
...
def has_tournament_permission(member, guild):
    """Check if member has tournament management permission (admins, host roles, or tournament leader roles)"""
    # Special user with all permissions
    if member.id == 1011738066962989056:  # @n_x_pedroz1's user ID
        return True
        
    # Check admin permissions
    if has_admin_permission(member, guild):
        return True
    
    # Check host permissions
    if has_host_permission(member, guild):
        return True
    
    # Check tournament leader roles
    guild_id = str(guild.id)
    if guild_id in tournament_leader_roles:
        member_role_ids = [role.id for role in member.roles]
        return any(role_id in member_role_ids for role_id in tournament_leader_roles[guild_id])

    return False

def has_admin_permission(member, guild):
    """Check if member has admin permission (administrator, assigned admin role, or special user)"""
    # Special user with all permissions
    if member.id == 1011738066962989056:  # @n_x_pedroz1's user ID
        return True
...
@bot.command()
async def tlr(ctx, role: discord.Role):
    """Set Tournament Leader Role - allows role to use all tournament commands"""
    try:
        await ctx.message.delete()
    except discord.NotFound:
        pass
    ...
    if not has_admin_permission(ctx.author, ctx.guild):
        return await ctx.send("❌ You don't have permission to set tournament leader roles.", delete_after=5)

    guild_id = str(ctx.guild.id)
    if guild_id not in tournament_leader_roles:
        tournament_leader_roles[guild_id] = []

    if role.id not in tournament_leader_roles[guild_id]:
        tournament_leader_roles[guild_id].append(role.id)
        await ctx.send(f"✅ {role.mention} can now manage tournaments (create, start, winner, code, fake, cancel)!", delete_after=5)
    else:
        await ctx.send(f"❌ {role.mention} already has tournament leader permissions.", delete_after=5)
...
async def start(ctx):
    ...
    if not has_tournament_permission(ctx.author, ctx.guild):
...
async def winner(ctx, member: discord.Member):
    ...
    if not has_tournament_permission(ctx.author, ctx.guild):
        return await ctx.send("❌ You don't have permission to record tournament winners.", delete_after=5)
    ...
    if len(tournament.results) == len(current_round):
        # Check if we need a bronze medal battle (when we have exactly 2 finalists)
        if len(tournament.results) == 2 and len(tournament.rounds) > 1:
            # Create bronze medal battle between the last 2 eliminated players/teams
            all_eliminated = tournament.eliminated
            bronze_battle_pairs = []

            if len(all_eliminated) >= 2:
                # Bronze battle between last 2 eliminated players
                bronze_battle_pairs.append((all_eliminated[-2], all_eliminated[-1]))

            if bronze_battle_pairs:
                # Create bronze medal battle round
                tournament.rounds.append(bronze_battle_pairs)
                old_results = tournament.results.copy()
                tournament.results = []

                embed = discord.Embed(
                    title=f"🏆 {tournament.title} - Battle for <:Medal_Bronze:1402383923991806063>",
                    description=f"**Map:** {tournament.map}\n**Mode:** {tournament.mode}\n**Abilities:** {tournament.abilities}",
                    color=0xcd7f32
                )

                for i, match in enumerate(bronze_battle_pairs, 1):
                    a, b = match
                    player_a = get_player_display_name(a, ctx.guild.id)
                    player_b = get_player_display_name(b, ctx.guild.id)
                    embed.add_field(
                        name=f"⚔️ Bronze Medal Battle",
                        value=f"**{player_a}** <:VS:1402690899485655201> **{player_b}**\n<:Crown:1409926966236283012> Winner: *Waiting...*",
                        inline=False
                    )

                embed.set_footer(text="Use !winner @player to record bronze medal battle result")

                # Store the finalists for later
                tournament.finalists = old_results
                active_tournament_view = discord.ui.View()
                tournament.message = await ctx.send(embed=embed, view=active_tournament_view)
                return

        elif len(tournament.results) == 1:
            # Bronze medal battle completed, now do the final
            bronze_winner = tournament.results[0]
            finalists = tournament.finalists

            # Create final match
            final_pairs = [(finalists[0], finalists[1])]
            tournament.rounds.append(final_pairs)
            tournament.results = []

            embed = discord.Embed(
                title=f"🏆 {tournament.title} - FINAL",
                description=f"**Map:** {tournament.map}\n**Mode:** {tournament.mode}\n**Abilities:** {tournament.abilities}",
                color=0xffd700
            )
            ...
            player_b = get_player_display_name(b, ctx.guild.id)
            embed.add_field(
                name=f"⚔️ FINAL MATCH",
                value=f"**{player_a}** <:VS:1402690899485655201> **{player_b}**\n<:Crown:1409926966236283012> Winner: *Waiting...*",
                inline=False
            )

            embed.set_footer(text="Use !winner @player to record final result")

            # Add winner's avatar if it's a real player
            if hasattr(bronze_winner, 'display_avatar') and not isinstance(bronze_winner, FakePlayer):
                embed.set_thumbnail(url=bronze_winner.display_avatar.url)

            # Store bronze winner for final rankings
            tournament.bronze_winner = bronze_winner
            active_tournament_view = discord.ui.View()
            tournament.message = await ctx.send(embed=embed, view=active_tournament_view)
            return
            ...
            # Calculate placements based on elimination order and bronze winner
            all_eliminated = tournament.eliminated
            bronze_winner = tournament.bronze_winner

            # Get the final placements
            placements = [] # List of (place, player_or_team, sp_reward)
            ...
            if hasattr(winner_data, 'id') and not isinstance(winner_data, FakePlayer):
                add_sp(ctx.guild.id, winner_data.id, 100)

            # 2nd place (final loser - last eliminated in final)
            if len(all_eliminated) >= 1:
                final_loser = all_eliminated[-1]
                placements.append((2, final_loser, 70))
                if hasattr(final_loser, 'id') and not isinstance(final_loser, FakePlayer):
                    add_sp(ctx.guild.id, final_loser.id, 70)

            # 3rd place (bronze medal winner)
            if bronze_winner:
                placements.append((3, bronze_winner, 50))
                if hasattr(bronze_winner, 'id') and not isinstance(bronze_winner, FakePlayer):
                    add_sp(ctx.guild.id, bronze_winner.id, 50)

            # 4th place (bronze medal loser)
            if len(all_eliminated) >= 2:
                bronze_loser = all_eliminated[-2]
                placements.append((4, bronze_loser, 30))
                if hasattr(bronze_loser, 'id') and not isinstance(bronze_loser, FakePlayer):
                    add_sp(ctx.guild.id, bronze_loser.id, 30)
            ...
            # Create results text with proper rankings
            results_display = ""
            for place, player_or_team, sp in placements:
                if place == 1:
                ...
                player_display = get_player_display_name(player_or_team, ctx.guild.id)
                place_suffix = "st" if place == 1 else "nd" if place == 2 else "rd" if place == 3 else "th"
                results_display += f"{emoji} **{place}{place_suffix} Place:** {player_display}\n"
            ...
            for place, player_or_team, sp in placements:
                ...
                place_suffix = "st" if place == 1 else "nd" if place == 2 else "rd" if place == 3 else "th"
                prize_text += f"{emoji} **{place}{place_suffix} Place:** {sp} SP\n"

            embed.add_field(name="🎁 Seasonal Points Prizes", value=prize_text, inline=False)

            # Add winner's avatar if it's a real player
            if hasattr(winner_data, 'display_avatar') and not isinstance(winner_data, FakePlayer):
                embed.set_thumbnail(url=winner_data.display_avatar.url)
...
async def code(ctx, code: str, member: discord.Member = None):
    ...
    # Check if user has permission to send codes
    if not has_tournament_permission(ctx.author, ctx.guild):
...
if __name__ == "__main__":
    if not TOKEN:
        ...
        try:
            ...
            print("Make sure you have added your Discord bot token to Secrets!")