...
# Load data
def load_data():
    global user_data, user_levels, leveling_settings, welcomer_settings, tp_data, bracket_roles, automod_settings
    try:
    ...
    for i, match in enumerate(current_round):
        if tournament.mode == "2v2":
            ...
            if member in team_a or member in team_b:
                ...
                eliminated_players.extend(eliminated_team)
                match_found = True
                match_index = i
                break
        else:
            ...
            if match_index >= 0 and match_index < len(current_embed.fields):
                ...
                if "Match" in field.name:
                    ...
                    if tournament.mode == "2v2":
                        ...
                        lines[1] = f"<:crown:1404513986887553157> Winner: **{winner_team_str}**"
                    else:
                        lines[1] = f"<:crown:1404513986887553157> Winner: **{get_player_display_name(member, ctx.guild.id)}**"
...
async def code(ctx, code: str, member: discord.Member = None):
    ...
    if member:
        ...
        code_message = f"🔐 **Tournament Room Code**\n```{code}```\n**Hosted by:** {host_name}\n\nGood luck in your match! 🎮"
