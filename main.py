        ...
        if len(tournament.results) == 2 and len(tournament.rounds) > 1:
            ...
            if len(all_eliminated) >= 1:
                ...
                if hasattr(player, 'id') and not isinstance(player, FakePlayer):
                    add_sp(ctx.guild.id, player.id, 70)
            ...
            if len(all_eliminated) >= 2:
                ...
                if hasattr(player, 'id') and not isinstance(player, FakePlayer):
                    add_sp(ctx.guild.id, player.id, 50)
            ...
            # Create results text without SP display (SP only shown in prizes)
            results_display = ""
            for place, player_or_team, sp in placements:
                ...
                results_display += f"{emoji} {player_display}\n"
            ...
            for place, player_or_team, sp in placements:
                ...
                prize_text += f"{emoji} {place}{place_suffix}: {sp} SP per player\n"
